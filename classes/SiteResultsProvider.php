<?php
    ...
    class SiteResultProvider
    {
        ...
        {
            $query = $this->con->prepare("SELECT COUNT(*) as total FROM sites WHERE title LIKE
             :term OR url LIKE :term OR keywords LIKE :term OR description LIKE :term");
        ...
        public function getResultsHtml($page, $pageSize, $term) // mamshu yerda tug'irlash kere tug'ri qidiruvni
        {
            ...
            $query = $this->con->prepare("SELECT * FROM sites WHERE title LIKE
             :term OR url LIKE :term OR keywords LIKE :term OR description LIKE :term 
             ...
             while ($row=$query->fetch(PDO::FETCH_ASSOC))
             {
                ...
                $title = $this -> trimField($title, $tnum);
                $description = $this -> trimField($description, $dnum);
